...
#### 像素之间的关系
* <span style="color:#ee0b41">页面不缩放的情况下，CSS 像素 == 设备独立像素 （前提是有声明，否则一个css像素还是对应一个物理像素） == 逻辑像素 == DIP == 位图像素</span>
...
像素比（device pixel ratio:DPR    dpr）： 单一方向上设备物理像素和设备独立像素的比例。
```js
...
@media screen and (-webkit-min-device-pixel-ratio: 2) {
    .logo {
        background-image: url('02day/image/logo@2x.png');
    }
}
```
...
- 布局视口：document.documentElement.clientWdith
- 视觉视口：window.innerWidth
- 理想视口：document.documentElement.clientWdith = window.innerWidth；
...
宽度与屏幕（设备独立像素）同宽的布局视口称为理想视口。 理想视口的好处
...
### PC 端
放大时
- 布局视口变小，视觉视口变小，相当于设备独立像素变大了

- 布局视口变小，视觉视口变小

- 元素的像素大小不变，但是视觉大小是在变大的

  

缩小时
- 布局视口变大，视觉视口变大，相当于设备独立像素变小了
- 布局视口变大，视觉视口变大
- 元素的像素大小不变，但是视觉大小是在变小的
...
viewport 相关选项
- width  布局视口宽度
- initial-scale  初始化缩放比例<font color="orange" size=4>**=设备独立像素宽度/布局视口宽度，通过设置initial-scale可以改变布局视口宽度=设备独立像素宽度/initial-scale**</font>
- minimum-scale  最小缩放比例
- maximum-scale 最大缩放比例
- user-scalable  设置是否允许用户缩放
- <span style="color:#ee0b41; font-size:20px">**viewport-fit   auto/contain/cover  针对刘海屏**</span>
...
<span style="color:red; font-weight:bolder; font-size:20px;">scale = 屏幕宽度独立像素  /  布局视口宽度</span>。 // iphone6  0.5  
...
#### <font color="orange">viewport-fit</font>
...
- touchmove 事件触发后，即使手指离开了元素（是指手指还在屏幕上，但是手指不在原来的元素身上了），touchmove 事件也会持续触发
...
<span style="color:#ee0b41">        touch 事件结束后会默认触发元素的<span style="color:orange;font-size:20px">click 事件</span></span>，如没有设置完美视口，则事件触发的时间间隔为 350ms 左右，如设置完美视口则时间间隔为 50ms 左右。<span style="color:orange;font-size=25px;font-weight:bolder">也就是touch时间触发一段时间后，会在原来的位置上再触发一次click事件，并不是冒泡</span>

​       **<font color="orange">如果 touch 事件隐藏了元素，则 click 动作将作用到新的元素上，</font>**触发新元素的 click 事件或页面跳转，此现象称为点击穿透
...
2、阻止顶级元素事件的默认行为，可以增加一个包裹元素绑定，**也可以给 document 和 window 绑定**，不过需要关闭被动模式（<font color="orange">如果passive为true，那么浏览器的默认行为则无法被阻止</font>）
...
<span style="color:orange; font-size:20px;">最外层元素阻止了 touchstart 默认行为之后，会产生一些意外现象😭</span>
...
产生『后遗症』的原因在于 touchstart 阻止了默认行为，后续所有的操作都已经失效。解决问题只需要给目标元素**绑定 touchstart 事件并阻止事件冒泡**，这样当前操作的默认行为仍然可用。<span style="color:orange; font-size:20px;">也就是在最外层来阻止默认行为，这样所有元素的touchstart的默认行为都被阻止了，如果想要保留某个元素的默认行为，就阻止这个元素冒泡，当点击没阻止冒泡的那些元素时，就会冒泡到外层包裹元素，然后阻止了默认行为。</span>
...
* <span style="color:orange;">changedTouches</span>：<span style="font-size:20px;font-weight:bolder">受不同手机的影响</span>
* <span style="color:orange;">targetTouches</span>
* <span style="color:orange;">touches</span>

1. touches属性返回事件触发时，表示当前屏幕上所有触点对象的集合。

2. targetTouches属性返回表示touchstart事件触发所在元素上所有触点对象的集合，即便触点已经移出此元素甚至document，照常返回。

3. changedTouches属性返回此次触屏事件与上次触屏事件发生变化的触点对象集合：

   （1）.对于touchstart事件，集合列出在此次事件中新增加触点对象。

   （2）.对于touchmove事件，集合列出和上一次事件相比较，发生了变化的触点对象。

   （3）.对于touchend事件，集合列出最后离开触摸平面的触点对象。

#### touchstart 事件

在 touchstart 事件中，

changedTouches  为<span style="color:orange;">**当前在元素**</span>上<span style="color:#ee0b41">同时按下</span>的触点对象数组。

targetTouches  为按下后，<span style="color:orange;">**当前元素上**</span>的触点对象数组

touches  为按下后，当前<span style="color:orange;">**屏幕上所有**</span>的触点对象数组
...
## 适配（不同机型上<span style="color:orange">等比缩放</span>）
...
拿到设计稿之后，设置布局视口（viewport）宽度为设计稿宽度，然后量尺寸进行布局即可。

> 但是直接拿width来控制布局视口宽度会有一定的兼容性问题。
>
> 在iphone和ipad上，无论你给viewport设的宽的是多少，如果没有指定默认的缩放值，则iphone和ipad会自动计算这个缩放值，以达到当前页面不会出现横向滚动条(或者说viewport的宽度就是屏幕的宽度)的目的。貌似现在大部分都是可以的。



### <span style="color:orange">rem适配</span>
...
rem 适配的策略有以下几种(前提是完美适口，也就是布局视口=视觉视口)
...
4. 增加 JS 代码进行页面适配。也就是如果机型的完美视口和设计稿不一样，那么就要对布局进行整体缩放。那么只需要计算现在的完美视口是设计稿的多少倍，然后对rem进行同比缩放。<span style="color:orange">html: font-size=100*doucment.documentElement.clientWidth/设计稿宽度。</span>
...
1. 完美视口设置
2. 通过 JS ，根据视觉视口大小（完美适口）设置页面的根元素字体大小，可以除以 10 也可以除以其他的数字
3. 根据设计稿使用 rem 设置元素大小。
4. 例如，设计稿的稿宽的1/10是1rem，那么不同机型的1rem=机子的完美适口宽度/10=document.documentElement.clientWidth/(设计稿宽度/设计稿的1rem)=设计稿的1rem*document.documentElement.clientWidth/设计稿宽度。<span style="color:orange; font-size:20px; font-weight:bolder">和方法一一样</span>

> 方法一是布局元素转化成rem好算，方法二是机型不同时，1rem好算。

### 1px 边框问题

高清屏幕下 1px 对应更多的物理像素，所以 1 像素边框看起来比较粗，解决方法如下

#### 方法一：在高倍屏中将一条边框按照dpr进行x或y方向的缩放
...
#### 方法二：首先利用initial-scale = 设备独立像素/布局视口大小，调整initial-scale可让布局视口大小等于屏幕物理像素，那么css中1px就对应一个物理像素。这样一来，所有内容都缩小了，那就把边框以外的尺寸全都放大对应倍数。
...
//为了使用rem适配的话，其实fontsize应该为：
document.documentElement.style.fontSize = fontSize * dpr * （html.clientWidth/设计稿宽度） + 'px';
```
